...
public class TourPlaceInfoService {
    ...
    public List<TourPlace> getList(TourPlaceSearch search) {
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody().getResponse().getHeader().getResultCode().equals("0000")) { // 여기서 가지고 온 데이터가 성공 데이터이면 또는 응답코드가 0000면

                List<Long> ids = response.getBody().getResponse().getBody().getItems().getItem().stream().map(ApiItem::getContentid).toList(); // contentId 항목으로 가지고 와서
                if (!ids.isEmpty()) {
                    QTourPlace tourPlace = QTourPlace.tourPlace;
                    List<TourPlace> items = (List<TourPlace>)repository.findAll(tourPlace.contentId.in(ids), Sort.by(asc("contentId"))); // 정렬 추가

                    return items; // 값을 내보낸다.
                } // endif
            } // endif
